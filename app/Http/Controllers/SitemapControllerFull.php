...
/**
 * BACKUP: Full multi-sitemap version with all locales, pagination, image sitemap.
 * This was the working version before switching to the lean ~560 URL sitemap.
 * To restore: rename this to SitemapController.php and restore the routes in web.php.
 */
...
class SitemapControllerFull extends Controller
{
    ...
    public function index(): Response
    {
        $sitemaps = Cache::remember('sitemap:index', self::CACHE_TTL, function () {
            ...
            // Models sitemaps (paginated, per locale) — distinct usernames only
            $totalModels = DB::connection('cam')->table('cam_models')->distinct()->count('username');
            $modelPages = max(1, ceil($totalModels / self::MODELS_PER_SITEMAP));

            // Get most recent model update for lastmod
            $lastModelUpdate = DB::connection('cam')->table('cam_models')->max('updated_at') ?? now();
    ...
    public function staticPages(): Response
    {
        ...
        // Tags index
        $urls[] = $this->buildUrl(url('/tags'), '0.8', 'daily', $this->buildLocaleAlternates('/tags'));

        // Countries index
        $urls[] = $this->buildUrl(url('/countries'), '0.7', 'weekly', $this->buildLocaleAlternates('/countries'));
     ...
     * Accepts params in any order since Laravel passes route params positionally.
     */
    public function models(string $pageOrLocale = '1', string $pageParam = '1'): Response
    {
        if (is_numeric($pageOrLocale)) {
            $page = (int) $pageOrLocale;
            $locale = null;
        } else {
            $locale = $pageOrLocale;
            $page = (int) $pageParam;
        ...
        $urls = Cache::remember($cacheKey, self::CACHE_TTL, function () use ($page, $locale) {
            ...
            // Deduplicate by username (models appear on multiple platforms),
            // keeping the row with highest engagement per username.
            $models = DB::connection('cam')
                ->table(DB::raw(
                    "(SELECT DISTINCT ON (username) id, username, updated_at, is_online, "
                    . "viewers_count, rating, favorited_count "
                    . "FROM cam_models "
    ...
    protected function calculateModelPriority(object $model): string
    {
        ...
        if (($model->favorited_count ?? 0) > 50000) {
            $priority += 0.05;
        }
    ...
    public function images(): Response
    {
        ...
        $content = Cache::remember($cacheKey, self::CACHE_TTL, function () {
            $models = DB::connection('cam')
                ->table(DB::raw(
                    "(SELECT DISTINCT ON (username) username, preview_url, avatar_url, updated_at, "
                    . "is_online, viewers_count "
                    . "FROM cam_models WHERE preview_url IS NOT NULL "
                    . "ORDER BY username, is_online DESC, viewers_count DESC NULLS LAST) AS deduped"
                ))
                ->orderByRaw('is_online DESC')
                ->orderByRaw('viewers_count DESC NULLS LAST')
            ...
            foreach ($models as $model) {
                $xml .= "    <url>\n";
                $xml .= "        <loc>" . htmlspecialchars(url("/model/{$model->username}")) . "</loc>\n";
    ...
    protected function renderSitemapIndex(array $sitemaps): string
    {
        ...
        foreach ($sitemaps as $sitemap) {
            ...
            $lastmod = $sitemap['lastmod'] instanceof \DateTimeInterface
                ? $sitemap['lastmod']->format(\DateTimeInterface::W3C)
                : Carbon::parse($sitemap['lastmod'])->toW3cString();
    ...
    /**
     * Routes that were used with this controller (for reference):
     *
     * Route::get('/sitemap.xml', [SitemapController::class, 'index']);
     * Route::get('/sitemap-static.xml', [SitemapController::class, 'staticPages']);
     * Route::get('/sitemap-models-{page}.xml', [SitemapController::class, 'models'])->where('page', '[0-9]+');
     * Route::get('/sitemap-models-{locale}-{page}.xml', [SitemapController::class, 'models'])->where(['locale' => '[a-z]{2}(-[A-Z]{2})?', 'page' => '[0-9]+']);
     * Route::get('/sitemap-tags.xml', [SitemapController::class, 'tags']);
     * Route::get('/sitemap-tags-{locale}.xml', [SitemapController::class, 'tags'])->where('locale', '[a-z]{2}(-[A-Z]{2})?');
     * Route::get('/sitemap-countries.xml', [SitemapController::class, 'countries']);
     * Route::get('/sitemap-countries-{locale}.xml', [SitemapController::class, 'countries'])->where('locale', '[a-z]{2}(-[A-Z]{2})?');
     * Route::get('/sitemap-niches.xml', [SitemapController::class, 'niches']);
     * Route::get('/sitemap-niches-{locale}.xml', [SitemapController::class, 'niches'])->where('locale', '[a-z]{2}(-[A-Z]{2})?');
